...
const COUNTRY_CODE = '+221'; // Sénégal (changez selon votre pays)
...
export default function PhoneScreen({ navigation }) {
  ...
  const handleSend = async () => {
    if (phone.replace(/\s/g, '').length < 7) {
      return Alert.alert('Numéro invalide', 'Veuillez saisir un numéro valide.');
    }
    setLoading(true);
    try {
      ...
      Alert.alert('Erreur', err.response?.data?.message || 'Impossible d\'envoyer le code. Réessayez.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <View style={styles.content}>
        ...
        <View style={styles.inputRow}>
          <View style={styles.flag}>
            <Text style={styles.flagCode}>{COUNTRY_CODE}</Text>
          </View>
          <TextInput
            style={styles.input}
        ...
        <Text style={styles.hint}>
          En développement, utilisez le code <Text style={styles.hintBold}>123456</Text>
        </Text>
...
const styles = StyleSheet.create({
  ...
  inputRow: {
    ...
    borderWidth: 2, borderColor: COLORS.gray[200], borderRadius: RADIUS.md,
    marginBottom: SPACING.sm, overflow: 'hidden',
  },
  flag: {
    paddingHorizontal: SPACING.md, paddingVertical: SPACING.md,
    backgroundColor: COLORS.gray[100], borderRightWidth: 1,
  ...
  input: {
    ...
    fontSize: 18, color: COLORS.secondary, letterSpacing: 2,
  },
